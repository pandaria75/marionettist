...
Skills 声明能力需求，如 `reasoning`、`coding`、`reflective` 或 `utility`。OpenCode agent 文件可能渲染具体的 `model` 字段，但这些值来自配置的 profiles，而不是在各个 skill 内部独立选择。默认 P1 工作流中，`review` profile 同时服务于 `harness-reviewer` 和 `harness-critic`。
