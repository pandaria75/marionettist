...
- In `balanced` mode, `/harness-continue` may continue only into the next already-approved `gateClass: simple` slice or group. In `autonomous` mode, it still pauses for `high-risk`, `boundary-sensitive`, critic-required, or explicitly requested gates. Final approval remains required by default.
