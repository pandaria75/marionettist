...
Read `.task/active.json` first, then `.task/<yyyy-MM-dd>/<task-slug>/state.json`.
...
- If `contextPackReady` is false, route to `context-pack-builder` and write `.task/<yyyy-MM-dd>/<task-slug>/context-pack.md`.
