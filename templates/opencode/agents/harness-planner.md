...
Do not implement production code. If writing a plan document is requested by the caller and allowed by the harness phase, write only the relevant `.task/<yyyy-MM-dd>/<task-slug>/implementation-plan.md`, `.task/<yyyy-MM-dd>/<task-slug>/state.json`, or context-pack planning content.
