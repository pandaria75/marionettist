...
description: Use when the user wants natural-language Pathway-scoped OpenCode or harness configuration help, especially opencode.json, harness.config.yaml, .harness/model-profiles.yml, permissions, gatePolicy, or tier-policy drafting with confirmation before writes.
---

# Harness Pathway Config

Pathway-scoped workflow for drafting configuration changes safely from natural-language requests.

Relationship note: `templates/opencode/commands/harness-config.md` is the general or legacy OpenCode config wrapper. This skill is Pathway-scoped guidance and must not silently duplicate or replace that wrapper.
...
1. Classify the request before drafting anything.
   ...
   - Route harness workflow gate settings to `harness.config.yaml` under `gatePolicy`.
   - Route model and temperature drafting only to `.harness/model-profiles.yml`.
   - Route `.harness/tier-policy.yml` changes only when the user explicitly asks for tier-policy authoring.
...
3. Keep `gatePolicy` separate from OpenCode permission settings.
   - Do not place harness gate-policy choices into OpenCode permission config.
   - Do not treat OpenCode permission rules as a substitute for harness workflow gates.
...
- the request implies `.harness/tier-policy.yml` edits but tier policy was not explicitly requested
...
- Drafting temperature changes anywhere other than `.harness/model-profiles.yml`
- Editing `harness.config.yaml` for OpenCode permission rules
- Editing OpenCode config to represent harness gate policy
- Touching `.harness/tier-policy.yml` without an explicit tier-policy request
...
- Any model or temperature change was drafted only in `.harness/model-profiles.yml`
...
- If the request is broader general configuration routing, hand off through the existing `harness-config` wrapper rather than silently merging the two surfaces.
