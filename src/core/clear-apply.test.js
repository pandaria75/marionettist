...
test("applyClearPlan backs up file before remove", async () => {
  ...
  assert.equal(
    await fs.readFile(path.join(projectPath, ".marionettist", "backups", "2026-06-16T10-20-30-000Z", "README.md"), "utf8"),
    "managed readme\n"
  );
});
...
test("applyClearPlan backs up file before managed-block edit", async () => {
  ...
  assert.equal(result.applied, true);
  assert.equal(
    await fs.readFile(path.join(projectPath, ".marionettist", "backups", "2026-06-16T10-20-31-000Z", "AGENTS.md"), "utf8"),
    agentsContent
  );
  ...
  assert.doesNotMatch(updatedAgents, /marionettist-kit:start/);
...
test("applyClearPlan dry-run makes no writes", async () => {
  ...
  assert.equal(await readIfExists(path.join(projectPath, ".marionettist", "backups", "2026-06-16T10-20-32-000Z", "README.md")), null);
});
...
test("applyClearPlan aborts mutation when backup fails", async () => {
  ...
  const blockingFile = path.join(projectPath, ".marionettist", "backups", "2026-06-16T10-20-33-000Z");
...
test("applyClearPlan excludes backup root targets from cleanup", async () => {
  const projectPath = await createProject([
    { path: ".marionettist/backups/old/README.md", source: "templates/README.md", kind: "file", hash: "old-backup-hash" },
    { path: "README.md", source: "templates/README.md", kind: "file", hash: "readme-hash" }
  ], {
    ".marionettist/backups/old/README.md": "preserve prior backup\n",
    "README.md": "managed readme\n"
  });
  ...
  assert.equal(await fs.readFile(path.join(projectPath, ".marionettist", "backups", "old", "README.md"), "utf8"), "preserve prior backup\n");
});
...
test("applyClearPlan keeps safe file for managed-block-only AGENTS", async () => {
  const projectPath = await createProject([
    ...
    "AGENTS.md": "<!-- marionettist-kit:start -->\n\nManaged only.\n\n<!-- marionettist-kit:end -->\n"
  });
