...
const mirrorChecks = [
  {
    source: templateOpencodePath("agents", "harness-planner.md"),
...
async function assertMirrorResyncFromTemplateSource() {
  ...
  try {
    ...
    assert((await fs.readFile(targetAbsolute, "utf8")) === modifiedSource, `${target} must be resynced from ${source}`);
  } finally {
    await fs.writeFile(sourceAbsolute, originalSource, "utf8");
    await harness("self", "init", "--apply", "--with-opencode");
    assert((await fs.readFile(targetAbsolute, "utf8")) === originalSource, `${target} must be restored after resetting ${source}`);
  }
}
