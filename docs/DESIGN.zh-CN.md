# Marionettist 设计文档

[English](./DESIGN.md)

本文档面向希望理解 Marionettist 存在的原因及其核心思想如何衔接的人。除非实现细节有助于解释设计决策，否则不在此赘述。

导航说明：

- 主文档地图见 [docs/README.md](./README.md)
- 稳定概念与维护者参考见 [docs/reference/README.md](./reference/README.md)
- gate、tier、slicing 等深入主题见 [docs/advanced/README.md](./advanced/README.md)

## 1. 定位

Marionettist 是一套面向使用 AI agent 的仓库的文件型协作框架。

它为希望 AI 工作满足以下条件的团队而设计：

- 可审查
- 可复现
- 可安全暂停和 review
- 不依赖单一 agent 产品
- 可随时间安全升级

它不是业务项目模板，不是 IDE 替代品，也不是绑定特定供应商的 agent 运行时。

## 2. 核心理念

AI 辅助开发的主要风险通常不是某一次糟糕的代码建议。更大的风险是上下文消失、约束漂移，以及 agent 在人类应当决策的位置越过了边界。

Marionettist 的应对方式是将协作契约放入仓库文件中。

重要事实不困在聊天里。它们存在于人类和 agent 都能审查的文件中。

## 3. 设计原则

### 3.1 文件即契约

Marionettist 使用普通文件承载持久状态：

- `AGENTS.md` 定义仓库级 agent 行为。
- `.aiassistant/rules/*.md` 存放可执行约束。
- `docs/**/*.md` 解释设计、工作流、所有权和风险。
- `.task/` 存放任务级的需求、计划、上下文和状态。
- `.marionettist/manifest.json` 追踪框架管理的资产。

普通文件便于 review、版本管理、diff 和同步。它们也让方法在不同 agent 间可移植。

### 3.2 Gate 比自主更重要

Marionettist 刻意不是一套"agent 一直做到完"的系统。

对非平凡工作，工作流在关键点停下：

1. 完成分析后，进入编码前
2. 每个已批准的实现 slice 或已批准 group 完成后

在同一个已批准 slice 内，编码可以流向 review。要移动超出该 slice，工作流必须尊重当前已选的 gate policy 和所有显式停止条件。

Gate policy 控制停顿行为。它不放宽安全规则、危险命令处理或范围边界。

### 3.3 工作应切片

大任务拆分为小的、可审查的单元更安全。

每个 slice 应定义：

- 目标
- 允许范围
- 禁止范围
- 验证期望
- 完成标准

这样审批、实现、review 和验证都聚焦在同一个工作单元上。

### 3.4 知识按需路由，不全量加载

文档应帮助 agent 理解相关区域。它们不应变成源码索引。

Marionettist 分离了：

- **docs**：解释设计含义、工作流、边界、风险和决策
- **rules**：定义行为约束
- **本地搜索 / IDE 工具**：查找文件、符号、调用点和实现细节

这使文档保持有用，避免变成过时目录。

### 3.5 核心必须保持项目中立

框架模板、skills 和 CLI 默认值不得假定某个业务领域、编程语言、构建工具、模块名称或客户规则。

项目特有知识应放在目标项目自身的 docs、rules、config 和 task 文件中。

## 4. 工作流模型

Marionettist 按任务复杂度伸缩流程。

- **Tier S**：平凡、低风险工作。直接编码和 review 可能足够。
- **Tier M**：标准范围工作。编码前准备任务上下文。
- **Tier L**：复杂或边界敏感工作。冻结需求、切片计划、构建上下文、使用更严格的 gate。

确切的任务状态契约会作为 `docs/project/marionettist-workflow.md` 安装到目标项目中。

设计意图很简单：做足够的分析让下一步编码安全，然后在正确的边界停止。

## 5. 资产所有权模型

框架分离受管资产和项目本地资产。

### 框架管资产

这些从本仓库安装，在 `.marionettist/manifest.json` 中追踪：

- 核心模板
- 标准规则
- 项目工作流文档
- 可复用 skills
- 可选的 OpenCode 脚手架

可通过 `marionettist diff` 预览、`marionettist sync` 更新。

### 项目本地资产

这些属于目标团队，默认必须保留：

- 本地 docs 和 rules
- `.task/` 中的任务工件
- `AGENTS.md` 中的项目本地块
- 本地 OpenCode 定制
- 不在 manifest 追踪范围内的文件

### `AGENTS.md` 的分区所有权

`AGENTS.md` 使用 managed 和 project-local 两个 block，使框架可以更新共享指引而不会覆盖本地团队规则。

## 6. CLI 设计

CLI 是文本资产的安全安装器和同步器。

其职责范围是窄的：

- 把 Marionettist 安装到目标项目
- 预览受管变更
- 应用安全的受管更新
- 默认保留本地内容
- 诊断所有权和漂移

CLI 不拥有业务知识，不自动解决所有冲突，不默认删除本地内容。

## 7. OpenCode 集成

OpenCode 是一个可选的执行层。

它添加了 slash commands、角色 agent、validator 指引和模型 profile 渲染。它不改变核心方法：仓库文件和 Marionettist gate 仍然是事实来源。

OpenCode 实践指南见 [docs/OPENCODE.zh-CN.md](./OPENCODE.zh-CN.md)。

## 8. 安全演进

Marionettist 必须能升级而不接管项目。

因此框架使用：

- manifest 追踪
- dry-run 预览
- managed block
- 保守的同步行为
- 非安全替换的显式 force 路径

安全演进比便利更重要。框架升级不应静默抹除本地团队实践。

## 9. 非目标

框架不追求：

- 替代 IDE 导航
- 生成代码索引
- 规定单一技术栈
- 接管项目私有知识
- 把团队绑定到单一 agent 供应商
- 绕过人类审批执行高风险决策
- 默认覆盖项目本地内容

## 10. 给框架维护者

本仓库是 framework 源。维护时请使用根目录 `AGENTS.md` 和 `.marionettist/self/` 下的 self-profile 规则。

不要把框架自维护规则复制到目标项目的模板中。
