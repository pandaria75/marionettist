...
本指南面向想在目标项目中安装和使用 harness 的技术负责人和开发者。
...
`harness init` 支持三种分发模式：

- `embedded` — 默认；harness 安装在目标仓库本地
- `hybrid` — 本地安装，同时带 adapter 感知元数据
- `adapter` — adapter 导向安装，同时保留本地生成文件的追踪

所选模式在适用时记录在 `.harness/manifest.json` 中。大多数团队从默认的 `embedded` 模式开始即可。
...
项目可以在 `harness.config.yaml` 中配置 gate 姿态：
...
确切的 gate 语义参照目标项目中安装的 `docs/project/harness-workflow.md`。
...
OpenCode 是可选的。没有它，harness 依然能通过文件、提示词和 skills 运行。

当你需要以下能力时使用 OpenCode：

- `/harness` 和聚焦的 slash commands
...
- 不要在本 framework 源码仓库中随意运行 `harness init`。这里要用 self 命令。
