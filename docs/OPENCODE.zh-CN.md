...
默认情况下，新安装推荐 builder-first 的最小命令面，以 `/harness` 为中心。`full` 模式会额外暴露 `/harness-feature`、`/harness-bugfix`、`/harness-refactor`、`/harness-context`、`/harness-status`、`/harness-continue` 这类显式高级 wrapper，供需要它们的团队使用。
...
### 默认 builder-first 入口：`/harness`

大多数请求都应优先使用它，尤其当用户只是用自然语言描述需求，而不是自己挑选低层命令时。

预期行为：

- 路由到 `harness-builder`
- 先把请求归类到正确的 harness workflow
- 在执行或委派前，用一句简洁的话说明选中的 workflow
- 只有在确实存在阻塞性歧义时，才提出最少量澄清问题

### 聚焦 wrapper：`/harness-dev`、`/harness-incident`、`/harness-docs`、`/harness-config`

当你想提示大致工作方向、但仍希望统一走 `harness-builder` 时，使用这些 wrapper。

- `/harness-dev`：开发、功能、实现或构建相关工作，不强迫用户先选高级命令
- `/harness-incident`：证据优先的 incident / 紧急修复调查
- `/harness-docs`：文档或写作工作
- `/harness-config`：harness 或项目 workflow 配置相关工作
...
- 不自动抓取 terminal logs
- 在编码前停下，并让 incident 工件为后续已批准的调查或实现步骤做好准备

### 高级 / full 模式逃生口

`full` 模式会保留下面这些显式高级 wrapper，供需要它们可见的团队使用：
...
这些命令依然有用，但应被视为 escape hatch，而不是普通用户的默认入口。`minimal` 安装会省略这些专用高级 wrapper，但 `/harness` 仍可路由这些意图。
