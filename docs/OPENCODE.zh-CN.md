...
harness init --with-opencode --opencode-command-surface full     # 高级
```
...
这些资产在 `.harness/manifest.json` 中追踪，参与 `harness diff` 和 `harness sync`。
...
最小命令面以 `/harness` 作为 builder-first 入口。Full 模式增加显式 wrapper：`/harness-feature`、`/harness-bugfix`、`/harness-refactor`、`/harness-context`、`/harness-status`、`/harness-continue`。
...
Agent 模型值从 `.harness/model-profiles.yml` 渲染，仅在需要时回退到 `harness.config.yaml` 的 profiles。Profile 与 skill 需求的映射：
...
2. 让 `harness diff` 和 `harness sync` 把值渲染到 `.opencode/agents/*.md`。
