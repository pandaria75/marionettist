...
| Agent | 原因 |
| --- | --- |
| `harness-builder` | 编排全流程，判断任务 tier，解读 gate 报告，防止任务漂移 |
...
请用能力强但性价比高的模型。这类 agent 的执行依据是 `.task/context-pack.md` 和 implementation plan 中已经写好的显式约束。
...
| Agent | 角色 | 模型层级 |
| --- | --- | --- |
| `harness-builder` | 主编排者。负责 analysis、执行 gates、选择已批准 slice、调用其他 agent，并在需要时停下等待确认。 | 第一层 |
...
安装后的 `.opencode/agents/*.md` 里的值都只是示例。每个 agent 角色都有自己的文件，你可以在其中设定模型。
